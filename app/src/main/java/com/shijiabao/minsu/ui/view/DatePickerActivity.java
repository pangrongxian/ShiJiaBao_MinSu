...
import com.shijiabao.minsu.datepicker.adapter.CalGridViewAdapter;
import com.shijiabao.minsu.datepicker.month.MyCalendar;
...
public class DatePickerActivity extends Activity implements MyCalendar.OnDaySelectListener {
            ...
            if(inday.equals(date)){
                ...
                try {
                    if(simpleDateFormat.parse(date).getTime()<simpleDateFormat.parse(inday).getTime()){
                        view.setBackgroundColor(Color.WHITE);
                        textDayView.setTextColor(getResources().getColor(R.color.color_default));
                        Toast.makeText(this, "离开日期不能小于入住日期", Toast.LENGTH_SHORT).show();
                        return;
                    }
                } catch (ParseException e) {
                    e.printStackTrace();
